...
def plot(predictions_file, app, output_dir):
  ...
  error_label = "Error (ms)"
  data = pd.read_csv(predictions_file, names=[predicted_label, actual_label, error_label])
  print(data)
#   input()
  ...
  fig, ax = plt.subplots()
  plt.scatter(x=predicted_label, y=actual_label, data=data, s=25, linewidth=0.05, alpha=0.5, color="red")
  plt.errorbar(data[predicted_label], data[actual_label], yerr=data[error_label], ls='none', fmt="--")
  ax.set_title(title)
  ...
  plt.plot([0, max], [0, max], linewidth=1, color="blue")
