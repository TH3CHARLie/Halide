...
benchmark_sample() {
    ...
    do
        ...
        HL_NUM_THREADS=20 \
            ...
            ${D}/bench \
            --estimate_all \
            --benchmarks=all --benchmark_min_time=10  \
                | tee ${D}/bench_${i}.txt || echo "Benchmarking failed or timed out for ${D}"

        # Add the runtime, pipeline id, and schedule id to the feature file
        R=$(cat ${D}/bench_${i}.txt | head -n1 | cut -d' ' -f8)
        python3 /home/xuanda/dev/Halide/training_scripts/parse_runtime.py --input ${D}/bench_${i}.txt --output ${D}/runtimes_${i}.txt
    done
    P=$3
    S=$2
    FNAME=$4
    ...
    ls -d ${D}/runtimes_*.txt | python3 /home/xuanda/dev/Halide/training_scripts/aggregate_runtime.py ${D}/runtimes.txt
